...
abstract class AAA { 
	int testVar; 			// 멤버변수
	abstract void test1(); 	// 추상메서드(abstract키워드 생략 불가능)	
	void test2() {} 		// 일반메서드
}
...
// 인터페이스는 implements를 작성해 인터페이스를 '구현'한다.(상속X)
class DDD implements BBB, CCC { // 다중 인터페이스 구현이 가능하다.
