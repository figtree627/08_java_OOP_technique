...
class Music extends Thread {
	...
	void playMusic() {
		for (int i = 0; i < 10; i++) {
			System.out.println("음악프로그램이 실행중");
		}
	}
	*/
	// Thread클래스로부터 제공되는 run()메서드며, 병렬적으로 수행할 명령어를 작성한다.
	
	public void run() {
			for (int i = 0; i < 10; i++) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				System.out.println(i+"번째 음악프로그램이 실행중");
			}	
		}
}
...
class Game implements Runnable {
	...
	public void run() {
		for (int i = 0; i < 10; i++) {
			try {Thread.sleep(450);} catch (InterruptedException e) {e.printStackTrace();}
			System.out.println(i+ "번째 게임 실행중");
		}	
	}
